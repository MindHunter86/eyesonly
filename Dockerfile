...
WORKDIR /usr/sources/onetimesecret
COPY . .
...
RUN echo "ready" \
  ...
  && CGO_ENABLED=1 GOOS=linux GOARCH=amd64 go build -pgo=merged.pprof -tags netgo,osusergo -trimpath -ldflags="-s -w -X 'main.version=$MAIN_VERSION' -X 'main.buildtime=$MAIN_BUILDTIME'" -o onetimesecret cmd/onetimesecret/*.go \
  && upx -9 -k onetimesecret \
  && ls -lah onetimesecret
...
COPY --from=build --chmod=0555 /usr/sources/onetimesecret/onetimesecret onetimesecret

USER nobody
ENTRYPOINT ["/usr/local/bin/onetimesecret"]
CMD ["--help"]
